...
namespace numl.Supervised
{
    public class DecisionTreeGenerator : Generator
    {
        ...
        public DecisionTreeGenerator(int depth = 5, int width = 2, Descriptor descriptor = null,
            Type impurityType = null, double hint = double.Epsilon)
        {
            if (width < 2)
        ...
        private Node BuildTree(Matrix x, Vector y, int depth, List<int> used)
        {
            ...
            foreach (var segment in measure.Segments)
            {
                var edge = new Edge()
                {
                    Parent = node,
                ...
                if (edge.Discrete)
                {
                    ...
                    slice = x.Indices(v => v[col] == segment1.Min);
                }
                else
                {
                    // get range label
                    edge.Label = $"{segment.Min} ≤ x < {segment.Max}";
                    // do range check for matrix slicing
                    slice = x.Indices(v => v[col] >= segment.Min && v[col] < segment.Max);
                }

                // if this number is 0 then this edge leads to a dead end - the edge will not be built
                if (slice.Any())
                {
                    Vector ySlice = y.Slice(slice);
                    edge.Child = ySlice.Distinct().Count() == 1 ? BuildLeafNode(ySlice[0]) : BuildTree(x.Slice(slice), ySlice, depth - 1, used);
                    edges.Add(edge);
                }
            }
